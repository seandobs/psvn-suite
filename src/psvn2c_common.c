...
#ifndef PSVN2C_COMMON_C
#define PSVN2C_COMMON_C
...
// Allows for direct manipulation of abstraction value map
// Added on 09/16 by Sean Dobson, sdob167@aucklanduni.ac.nz
static void abstraction_map_domain_value( abstraction_t *abst, const int domain, const int old_value, const int new_value )
{

  if(domain < 0 || domain >= NUMDOMAINS) {
    fprintf(stderr, "Domain index out of bounds\n");
  }

  int i;
  for (i = 0; i < domain_sizes[ domain ]; ++i) {
    if(abst->value_map[domain][i] == old_value)
      abst->value_map[domain][i] = new_value;
  }

}

// Returns the number of domains
static int get_numdomains()
{
  return NUMDOMAINS;
}

static int get_numvars()
{
  return NUMVARS;
}

// Returns the size of the given domain
static int get_domain_size(const int domain)
{
  if(domain < 0 || domain >= NUMDOMAINS) {
    fprintf(stderr, "Domain index out of bounds\n");
  }
  return domain_sizes[domain];
}

// Returns the value map of the a given abstraction
static const int8_t* get_abstraction_value_map( const abstraction_t *abst)
{
  return abst->value_map[0];
}
