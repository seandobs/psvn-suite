...
typedef struct {
  ...
  // Exposes low-level access to abstraction functions.
  // Added on 09/16 by Sean Dobson, sdob167@aucklanduni.ac.nz
  void (*abstraction_map_domain_value)(void*, int, int, int);
  int (*get_numdomains)();
  int (*get_numvars)();
  int (*get_domain_size)(int);
  const int8_t* (*get_abstraction_value_map)(const void*);
...
#define allocate_so_abstraction( compiled_game_so_ptr ) (compiled_game_so_ptr)->allocate_abstraction()
...
// Exposes low-level access to abstraction functions.
  // Added on 09/16 by Sean Dobson, sdob167@aucklanduni.ac.nz
#define so_abstraction_map_domain_value( abstraction_ptr, domain, old_value, new_value, compiled_game_so_ptr ) (compiled_game_so_ptr)->abstraction_map_domain_value(abstraction_ptr, domain, old_value, new_value)
#define so_get_numdomains(compiled_game_so_ptr) (compiled_game_so_ptr)->get_numdomains()
#define so_get_numvars(compiled_game_so_ptr) (compiled_game_so_ptr)->get_numvars()
#define so_get_domain_size(domain, compiled_game_so_ptr) (compiled_game_so_ptr)->get_domain_size(domain)
#define so_get_abstraction_value_map( abstraction_ptr, compiled_game_so_ptr ) (compiled_game_so_ptr)->get_abstraction_value_map(abstraction_ptr)
